...
Decomposed into focused modules (Session 6):
  - ctrl_util.py       — log, now_iso, JSON I/O helpers
  - trade_logger.py    — Telegram alerts, rejection/near-miss/decision logs
  - position_manager.py — Position/TradeResult dataclasses, reconciliation
  - risk_gate.py       — 9 risk checks, _StrategyParams, halt logic

This file retains: Controller class, SessionState, open_trade, close_trade,
trailing stops, run_once cycle, main loop.

All extracted names are re-exported for backward compatibility.
...
# ── Extracted modules (Session 6) ────────────────────────────────────────────
from scanner.v6.ctrl_util import (                          # noqa: F401
    log, now_iso, load_json, save_json_atomic, append_jsonl, update_heartbeat,
)
from scanner.v6.trade_logger import (                       # noqa: F401
    send_alert, log_rejection, log_near_miss, log_decision,
    REJECTION_LOG_FILE, NEAR_MISS_LOG_FILE, DECISION_LOG_FILE,
    _alert_history,
)
from scanner.v6.position_manager import (                   # noqa: F401
    Position, ExecutionQuality, TradeResult,
    _safe_save_positions, _reconcile_positions,
)
from scanner.v6.risk_gate import (                          # noqa: F401
    _StrategyParams, approve_entry, _approve_entry_inner,
    inject_strategy_params, check_time_exits, handle_entry_end_events,
    load_risk, save_risk, check_halt, get_equity,
    _today_start, _get_current_regime,
)
        ...
        for coin, sig, direction, reason, entry in rejected:
            ...
            if consensus is not None and "consensus_threshold" in reason:
                ...
                if int(consensus) >= threshold - 2:
                    log_near_miss(entry, reason, params.name)
